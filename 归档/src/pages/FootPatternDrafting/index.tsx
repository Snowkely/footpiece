...
import PatternDxfExportControls from './components/PatternDxfExportControls';
...
const FootPatternDrafting: React.FC = () => {
    ...
    const finalFrontContourResult = useMemo<GeometryBuildResult<FinalFrontCutContour>>(() => {
        if (!frontPieceResult.geometry || !targetMultiSupportOuterCurveResult.geometry) {
            return { errors: [] };
        }

        return buildFinalFrontCutContour(
            frontPieceResult.geometry,
            targetMultiSupportOuterCurveResult.geometry,
        );
    }, [frontPieceResult.geometry, targetMultiSupportOuterCurveResult.geometry]);

    const finalBackContourResult = useMemo<GeometryBuildResult<FinalCutContour>>(() => {
        if (!completedBackPieceResult.geometry?.points.FPrime) {
            return { errors: [] };
        }

        return buildFinalBackCutContour(completedBackPieceResult.geometry);
    }, [completedBackPieceResult.geometry]);

    const finalPatternExportResult = useMemo<
        GeometryBuildResult<FinalPatternExportGeometry>
    >(() => {
        if (
            !frontPieceResult.geometry ||
            !completedBackPieceResult.geometry?.points.FPrime ||
            !targetMultiSupportOuterCurveResult.geometry
        ) {
            return { errors: [] };
        }

        return buildFinalPatternExportGeometry({
            frontPiece: frontPieceResult.geometry,
            completedBackPiece: completedBackPieceResult.geometry,
            manualCandidate: targetMultiSupportOuterCurveResult.geometry,
        });
    }, [
        completedBackPieceResult.geometry,
        frontPieceResult.geometry,
        ...
        <main className="foot-drafting-page">
            ...
            <div className="foot-drafting-workspace">
                <aside className="foot-drafting-column foot-drafting-input-column">
                    ...
                    />
                    <PatternDxfExportControls
                        manualCandidate={targetMultiSupportOuterCurveResult.geometry}
                        frontContourReady={Boolean(finalFrontContourResult.geometry)}
                        backContourReady={Boolean(finalBackContourResult.geometry)}
                        exportGeometry={finalPatternExportResult.geometry}
                        errors={finalPatternExportResult.errors}
                    />
