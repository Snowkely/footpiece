...
function parseLwPolylines(dxfText: string): ParsedLwPolyline[] {
    ...
    for (let index = 0; index < pairs.length; index += 1) {
        ...
        entities.push({
            handle: entityPairs.find(([code]) => code === 5)?.[1] ?? '',
            owner: entityPairs.find(([code]) => code === 330)?.[1] ?? '',
...
function sectionNames(pairs: Array<[number, string]>): string[] {
    return pairs.flatMap((pair, index) =>
        pair[0] === 0 && pair[1] === 'SECTION' && pairs[index + 1]?.[0] === 2
            ? [pairs[index + 1][1]]
            : [],
    );
}

function modelSpaceBlockRecordHandle(pairs: Array<[number, string]>): string {
    for (let index = 0; index < pairs.length; index += 1) {
        if (pairs[index][0] !== 0 || pairs[index][1] !== 'BLOCK_RECORD') continue;
        const record = pairs.slice(
            index + 1,
            pairs.findIndex((pair, i) => i > index && pair[0] === 0),
        );
        if (record.some(([code, value]) => code === 2 && value === '*Model_Space')) {
            return record.find(([code]) => code === 5)?.[1] ?? '';
        }
    }
    return '';
}

describe('canonical AutoCAD 2007 Master DXF serializer', () => {
    it('writes a complete AC1021 millimeter drawing with two owned closed LWPOLYLINE entities', () => {
        ...
        expect(PRESSURE_STOCKING_DXF_VERSION).toBe('AC1021');
        ...
        expect(sectionNames(pairs)).toEqual([
            'HEADER',
            'CLASSES',
            'TABLES',
            'BLOCKS',
        ...
        [
            ...
            'DIMSTYLE',
            'BLOCK_RECORD',
        ].forEach((tableName) => expect(pairs).toContainEqual([2, tableName]));
        expect(pairs.at(-1)).toEqual([0, 'EOF']);
        expect(entities).toHaveLength(2);
        expect(entities.map((entity) => entity.layer)).toEqual([CUT_FRONT_LAYER, CUT_BACK_LAYER]);
        expect(entities.map((entity) => entity.handle)).toEqual(['20', '21']);
        const modelSpaceOwner = modelSpaceBlockRecordHandle(pairs);
        expect(modelSpaceOwner).not.toBe('');
        expect(entities.every((entity) => entity.owner === modelSpaceOwner)).toBe(true);
        ...
        expect(entities[0].points).toEqual(
            geometry.front.masterPointsMm.map(({ x, y }) => ({ x, y })),
        );
        expect(entities[1].points).toEqual(
            geometry.back.masterPointsMm.map(({ x, y }) => ({ x, y })),
        );
    });
